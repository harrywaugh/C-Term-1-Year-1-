...
void forward(list *l)
{
    ...
    {
        fprintf(stderr, "ERROR: Already at last position, can not go forwards.\n");
        exit(1);
    }
}
...
void backward(list *l)
{
    ...
    {
        fprintf(stderr, "ERROR: Already at first position, can not go backwards.\n");
        exit(1);
    }
}
...
int testLists()
{
    ...
    //TESTS 19 TO 21 - Checks a different type list(Char)
    list* chars_list = newList(sizeof(char *));
    //Add new node before current position
    char *char1 = "Node 1";
    insertBefore(chars_list, &char1);
    //Check length is now 1
    if(chars_list->length == 1) testCompleted++;
    else return testCompleted;
    //Insert new node after current position
    char *char2 = "Node 2";
    insertAfter(chars_list, &char2);
    //First move current position to inbetween the Node 1 and 2.
    start(chars_list);
    forward(chars_list);
    //Check Node 1's value is "Node 1";
    char *test_char = "forty-two";
    getBefore(chars_list, &test_char);
    if(strcmp(test_char, "Node 1") == 0) testCompleted++;
    else return testCompleted;
    //Check Node 2's value is "Node 2"
    getAfter(chars_list, &test_char);
    if(strcmp(test_char, "Node 2") == 0) testCompleted++;
    else return testCompleted;
